...
testUngradedItems.forEach((item) => {
  ...
  console.log(
    `${item.itemName.padEnd(20)} ${item.itemWeight
      .toString()
      .padStart(5)}%   ` +
      `${deduction.toFixed(4).padStart(8)}%  ${requiredPct
        .toFixed(4)
        .padStart(8)}%  ` +
...
const totalWeight = testUngradedItems.reduce(
...
testUngradedItems.forEach((item) => {
  ...
  console.log(
    `${item.itemName.padEnd(20)} ${item.itemWeight
      .toString()
      .padStart(5)}%   ` +
      `${share.toFixed(4)}   ${deduction.toFixed(4).padStart(8)}%  ${requiredPct
        .toFixed(4)
        .padStart(8)}%  ` +
      `${requiredScore.toFixed(2).padStart(6)}/100`
  );
...
const equalTotal = testUngradedItems.reduce((sum, item) => {
  const deduction = Math.min(
    totalDeductiblePoints / testUngradedItems.length,
    item.itemWeight
  );
...
console.log(
  `\nEqual Strategy Total Required: ${equalTotal.toFixed(
    4
  )}% + 12% (completed) = ${(equalTotal + 12).toFixed(2)}%`
);
console.log(
  `Proportional Strategy Total Required: ${propTotal.toFixed(
    4
  )}% + 12% (completed) = ${(propTotal + 12).toFixed(2)}%`
);
