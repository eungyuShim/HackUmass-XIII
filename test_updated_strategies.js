...
const { proportionalStrategy, equalStrategy } = require('./GradePlanner/app/types/strategy.ts');
...
const testData = {
  ungradedItems: [
    ...
    { itemName: 'homework', categoryId: 2, itemWeight: 30, isAttendance: false },
    ...
    // Attendance items
    { itemName: 'attendance1', categoryId: 4, itemWeight: 1, isAttendance: true },
    { itemName: 'attendance2', categoryId: 4, itemWeight: 1, isAttendance: true },
    { itemName: 'attendance3', categoryId: 4, itemWeight: 1, isAttendance: true },
    { itemName: 'attendance4', categoryId: 4, itemWeight: 1, isAttendance: true },
    { itemName: 'attendance5', categoryId: 4, itemWeight: 1, isAttendance: true },
    { itemName: 'attendance6', categoryId: 4, itemWeight: 1, isAttendance: true },
    { itemName: 'attendance7', categoryId: 4, itemWeight: 1, isAttendance: true },
    { itemName: 'attendance8', categoryId: 4, itemWeight: 1, isAttendance: true },
    { itemName: 'attendance9', categoryId: 4, itemWeight: 1, isAttendance: true },
    { itemName: 'attendance10', categoryId: 4, itemWeight: 1, isAttendance: true },
  ],
...
try {
  ...
  regular.slice(0, 5).forEach(item => {
    console.log(`  ${item.itemName.padEnd(15)} Weight: ${item.itemWeight.toFixed(2)}%  Score: ${item.assumedScore.toFixed(2)}/100`);
  });
  ...
  const totalRequired = proportionalResult.reduce((sum, item) => {
    const score = item.isAttendance ? (item.assumedScore === 1 ? 100 : 0) : item.assumedScore;
  ...
  console.log(`Match: ${Math.abs(12 + totalRequired - 93) < 0.1 ? '✅ YES' : '❌ NO'}`);
